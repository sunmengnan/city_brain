...
from nets.cnn.paper_cnn16 import PaperCNN16
from nets.cnn.deep_cnn import DeepCNN
...
class CRNN(object):
    ...
    def __init__(self, cfg, num_classes):
        ...
        self.seq_len = tf.placeholder(tf.int64, name="sequence_length")
    ...
    def _build_model(self):
        ...
        if self.cfg.name == 'raw16':
            net = PaperCNN16(self.inputs, self.is_training)
        if self.cfg.name == 'deep_raw':
            net = DeepCNN(self.inputs, self.is_training)
    ...
    def _build_train_op(self):
        ...
        if reg_losses is not None and len(reg_losses) != 0:
            self.regularization_loss = tf.add_n(reg_losses)
            self.total_loss = self.ctc_loss + self.regularization_loss

            tf.summary.scalar('regularization_loss', self.regularization_loss)
        else:
            self.total_loss = self.ctc_loss
